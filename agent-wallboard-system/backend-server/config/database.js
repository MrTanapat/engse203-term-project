...
const SQLITE_DB_PATH =
  process.env.SQLITE_DB_PATH || "./database/sqlite/wallboard.db";
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!fs.existsSync(dbDir)) {
      ...
      try {
        ...
        reject(
          new Error(`Failed to create database directory: ${error.message}`)
        );
        return;
      }
    }
      ...
      if (err) {
        ...
        db.get("SELECT COUNT(*) as count FROM agents", (err, row) => {
          if (err) {
            ...
            db.close();
            reject(
              new Error("Database schema error - please run setup script")
            );
          } else {
...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://localhost:27017/wallboard";
...
async function connectMongoDB() {
  ...
  while (currentRetry < maxRetries) {
    try {
      ...
      currentRetry++;
      console.error(
        `❌ MongoDB connection attempt ${currentRetry}/${maxRetries} failed:`,
        error.message
      );
