...
import { Logout, People, Message, Refresh, Circle } from "@mui/icons-material";
...
function Dashboard({
  ...
  const filteredAgents = safeTeamData.filter(
    (agent) => statusFilter === "All" || agent.currentStatus === statusFilter
  );
  ...
  const teamStats = {
    ...
    available: safeTeamData.filter((a) => a.currentStatus === "Available")
      .length,
