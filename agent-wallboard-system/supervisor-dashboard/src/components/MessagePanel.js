...
function MessagePanel({ open, onClose, supervisor, teamData, onSendMessage }) {
  ...
  const onlineAgents = teamData
    ? teamData.filter((agent) => agent.isOnline)
    : [];
    ...
    <Dialog open={open} onClose={handleCancel} maxWidth="sm" fullWidth>
      ...
      <DialogContent>
        ...
        <FormControl component="fieldset" sx={{ mb: 3, mt: 1 }}>
          ...
          <RadioGroup row value={messageType} onChange={handleTypeChange}>
            <FormControlLabel
              value="direct"
              control={<Radio />}
      ...
      <DialogActions>
        ...
        <Button
          ...
          disabled={
            !messageContent.trim() ||
            (messageType === "direct" && !selectedAgent)
          }
        >
          Send Message
        </Button>
